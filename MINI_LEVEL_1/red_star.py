...
print(f"Директория скрипта red_star.py (для загрузки его ресурсов): {CURRENT_DIR}")

# Вспомогательная функция для получения полного пути к ресурсу
CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
...
class IntroView(arcade.View):
    def __init__(self):
        super().__init__()
        db(5)
        self.texture = arcade.load_texture(get_asset_path("red_star.png"))
        ...
        self.wait_time = 15.0
        self.sound_red_star = arcade.load_sound(get_asset_path('sound_red_star.mp3'))
        self.sound_red_star.play()

    def on_update(self, delta_time):
        self.display_timer += delta_time
        if self.ship.center_x > 1200:
            # Плавно меняем угол до 135 градусов (наклон вверх и вправо)
            if self.ship.angle > -90:
                self.ship.angle -= 40 * delta_time
                self.ship.center_y += 50 * delta_time
                self.ship.center_x += 150 * delta_time
            else:
                self.ship.center_y += 50 * delta_time
        else:
            self.ship.center_x += 150 * delta_time
        cam_x = self.ship.center_x
        if cam_x < 800:
            cam_x = 800
        elif cam_x > 1596:
            cam_x = 1596
        self.world_camera.position = (cam_x, SCREEN_HEIGHT // 2)
        if self.display_timer >= self.wait_time:
            game_view = MyGame()
    ...
    def on_draw(self):
        ...
        with self.world_camera.activate():
            arcade.draw_texture_rect(
                rect=arcade.rect.XYWH(1198, 375, 2396, 750),
...
class MyGame(arcade.View):
    def __init__(self):
        super().__init__()
        self.texture = arcade.load_texture(get_asset_path("baze_red.png"))
        ...
        self.full_text = ("Ошибка модуля терморегуляции.\n"
                          "Температура корпуса достигла критической отметки.\n"
                          "Требуется ручной запуск системы отопления корабля.")
        self.baze_img = arcade.load_texture(get_asset_path("baze.jpg"))
        self.setup()

    """Метод для запуска игры. Обнуляет всё состояние."""
                ...
                if not self.activated:
                    ...
                    def on_change(event):
                        if self.ui_slider.value >= 98:
                            ...
                            self.ui()  # Закрыть
                            self.ui("batery")  # Открыть обновленным
                else:
                    self.v_box.add(
    ...
    def draw_message(self):
        ...
        arcade.draw_rect_filled(arcade.rect.XYWH(800, 375, 1600, 750), (156, 222, 255, 90))
        arcade.draw_rect_filled(arcade.rect.XYWH(800, 640, 540, 140), (0, 0, 0, 150))
    ...
    def game_win(self):
        ...
        def on_click_forward(event):
            from MINI_LEVEL_2.Blue_star import Intro_blue
            next_level_view = Intro_blue()
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        """Проверка клика по батарейке"""
        if not self.game_flag:
            if (475 < x < 723.5 and 160 < y < 241):
                self.setup()
                return
            elif (873 < x < 1123.5 and 160 < y < 239):
                arcade.close_window()
                return
        if self.visible:
            return
        ...
        if self.split.collides_with_point(a):
            self.ui("split")
        if self.batery.collides_with_point(a) and self.game_timer >= 10.0:
            self.ui("batery")
